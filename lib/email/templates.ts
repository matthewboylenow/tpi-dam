...
export function getPasswordResetEmailHtml(params: PasswordResetEmailParams): string {
  const { recipientEmail, resetUrl, expiresAt } = params;
...
<head>
  ...
  <title>Reset Your Password - Taylor Products DAM</title>
</head>
          ...
          <tr>
            <td style="padding: 40px;">
              ...
              <p style="margin: 0 0 24px; color: #475569; font-size: 16px; line-height: 1.6;">
                Click the button below to choose a new password. This link will expire on <strong>${expiryDate}</strong>.
              </p>
              ...
              <!-- Warning Box -->
              <table role="presentation" style="width: 100%; background-color: #fff7ed; border-left: 4px solid #f97316; border-radius: 4px;">
                <tr>
...
export function getPasswordResetEmailText(params: PasswordResetEmailParams): string {
  const { recipientEmail, resetUrl, expiresAt } = params;
  const expiryDate = expiresAt.toLocaleString("en-US", {
    month: "long",
    day: "numeric",
  ...
  return `
Reset Your Password - Taylor Products DAM
...
export function getWelcomeEmailHtml({ recipientName, recipientEmail, role }: WelcomeEmailParams): string {
  const dashboardUrl = (process.env.NEXTAUTH_URL || "http://localhost:3000") + "/dashboard";
  return `<!DOCTYPE html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Welcome to Taylor Products DAM</title></head>
<body style="margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;background-color:#f8fafc;">
<table role="presentation" style="width:100%;border-collapse:collapse;"><tr><td align="center" style="padding:40px 0;">
<table role="presentation" style="width:600px;max-width:100%;background-color:#ffffff;border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
<tr><td style="background:linear-gradient(135deg,#1C4E80 0%,#2F6FB2 100%);padding:40px 40px 30px;text-align:center;border-radius:8px 8px 0 0;">
<h1 style="margin:0;color:#ffffff;font-size:28px;font-weight:700;">Taylor Products DAM</h1>
<p style="margin:10px 0 0;color:#e2e8f0;font-size:14px;">Digital Asset Management</p></td></tr>
<tr><td style="padding:40px;">
<h2 style="margin:0 0 20px;color:#1e293b;font-size:24px;font-weight:600;">Welcome, ${recipientName}!</h2>
<p style="margin:0 0 16px;color:#475569;font-size:16px;line-height:1.6;">Your account has been created successfully. You are signed up as ${role === "admin" ? "an <strong>Administrator</strong>" : "a <strong>Sales Team Member</strong>"}.</p>
<p style="margin:0 0 24px;color:#475569;font-size:16px;line-height:1.6;">Click below to sign in and start managing digital assets.</p>
<table role="presentation" style="margin:0 0 24px;"><tr><td style="border-radius:6px;background:linear-gradient(135deg,#2F6FB2 0%,#1C4E80 100%);">
<a href="${dashboardUrl}" style="display:inline-block;padding:14px 32px;color:#ffffff;text-decoration:none;font-size:16px;font-weight:600;border-radius:6px;">Go to Dashboard</a>
</td></tr></table>
<p style="margin:0;color:#94a3b8;font-size:13px;">This account was created for ${recipientEmail}. If this wasn't you, please contact your administrator.</p>
</td></tr>
<tr><td style="padding:24px 40px;background-color:#f8fafc;text-align:center;border-radius:0 0 8px 8px;border-top:1px solid #e2e8f0;">
<p style="margin:0;color:#94a3b8;font-size:12px;">© ${new Date().getFullYear()} Taylor Products. All rights reserved.</p>
</td></tr></table></td></tr></table></body></html>`.trim();
}

export function getWelcomeEmailText({ recipientName, recipientEmail, role }: WelcomeEmailParams): string {
  const dashboardUrl = (process.env.NEXTAUTH_URL || "http://localhost:3000") + "/dashboard";
  return `Welcome to Taylor Products DAM, ${recipientName}!

Your account has been created as ${role === "admin" ? "an Administrator" : "a Sales Team Member"}.

Sign in at: ${dashboardUrl}

This account was created for ${recipientEmail}.

© ${new Date().getFullYear()} Taylor Products. All rights reserved.`.trim();
}
